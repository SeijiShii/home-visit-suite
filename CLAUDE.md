...
- **仕様変更は `/spec-change` スキルで進める**: 仕様追加・機能改修・挙動変更の依頼では、`.claude/skills/spec-change` の一貫フロー（wants 改訂→CODEMAP 参照で実装→CODEMAP 同期→i18n レビュー→同一コミット）に沿う。中間ドキュメントは作らない軽量版。
