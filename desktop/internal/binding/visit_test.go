...
// User-A は member ロール、co-1 はアクティブなチェックアウト。
func setupVisitBinding(t *testing.T) (*binding.VisitBinding, *repository.InMemoryCheckoutRepository) {
	t.Helper()
...
func TestVisitBinding_RecordVisit_Met(t *testing.T) {
	...
	now := time.Now()
	vr, err := b.RecordVisit("user-A", "co-1", "place-1", models.VisitResultMet, now, "")
	if err != nil {
...
func TestVisitBinding_RecordVisitPhase1_Met(t *testing.T) {
	...
	if vr.CheckoutID != "" {
		t.Errorf("CheckoutID = %q, want empty (Phase 1)", vr.CheckoutID)
	}
...
func TestVisitBinding_ListMyVisitHistory(t *testing.T) {
	...
	for _, vr := range []*models.VisitRecord{
		{ID: "h1", PlaceID: "place-1", UserID: "user-A", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultMet, VisitedAt: now.Add(-72 * time.Hour)},
		{ID: "h2", PlaceID: "place-1", UserID: "user-A", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultAbsent, VisitedAt: now.Add(-24 * time.Hour)},
		{ID: "h3", PlaceID: "place-1", UserID: "user-B", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultMet, VisitedAt: now},
	} {
...
func TestVisitBinding_GetLastMetDate(t *testing.T) {
	...
	for _, vr := range []*models.VisitRecord{
		// 古い met
		{ID: "v1", PlaceID: "p1", UserID: "user-X", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultMet, VisitedAt: now.Add(-30 * 24 * time.Hour)},
		// 新しい absent (会えていない、対象外)
		{ID: "v2", PlaceID: "p1", UserID: "user-Y", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultAbsent, VisitedAt: now.Add(-1 * 24 * time.Hour)},
		// 中間の met（最も新しい met）
		{ID: "v3", PlaceID: "p1", UserID: "user-Z", AreaID: "area-1", CheckoutID: "co-1", Result: models.VisitResultMet, VisitedAt: now.Add(-7 * 24 * time.Hour)},
	} {
		repo.SaveVisitRecord(vr)
	}
...
func TestVisitBinding_GetLastMetDate_NoMet(t *testing.T) {
	...
	repo.SaveVisitRecord(&models.VisitRecord{
		ID: "v-only-absent", PlaceID: "p2", UserID: "user-A", AreaID: "area-1", CheckoutID: "co-1",
