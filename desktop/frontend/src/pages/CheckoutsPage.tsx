...
async function buildRegionMaps(): Promise<RegionTreeMaps> {
...
export function CheckoutsPage() {
  ...
  const [invitations, setInvitations] = useState<models.CheckoutInvitation[]>(
    [],
  );
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [maps, all, allUsers] = await Promise.all([
        buildRegionMaps(),
        CheckoutBinding.ListAllCheckouts(),
    ...
    async function loadDetail() {
      try {
        const [invs, records] = await Promise.all([
          CheckoutBinding.ListInvitations(selectedId as string),
          selected
            ? VisitBinding.ListVisitRecords(selected.areaId)
            : Promise.resolve([] as models.VisitRecord[]),
        ]);
        if (cancelled) return;
        setInvitations(invs ?? []);
        // 当該チェックアウト ID に紐づく訪問記録のみカウント
        const filtered = (records ?? []).filter(
          (r) => r.checkoutId === selectedId,
        );
  ...
  const handleReturn = useCallback(async () => {
    if (!selected) return;
    try {
      await CheckoutBinding.Return(currentActorID, selected.id);
      await reload();
    } catch (e) {
      setError(`${c.actionReturn}: ${String(e)}`);
    }
  }, [selected, currentActorID, reload, c.actionReturn]);
  ...
  const handleForceReturn = useCallback(async () => {
    ...
    try {
      await CheckoutBinding.ForceReturn(currentActorID, selected.id);
      await reload();
    } catch (e) {
  ...
  const handleRevokeInvite = useCallback(
    async (inv: models.CheckoutInvitation) => {
      try {
        await CheckoutBinding.RevokeInvite(currentActorID, inv.id);
        // selectedId 経由で再ロード
        if (selectedId) {
          const invs =
    ...
    [currentActorID, selectedId, c.revokeInvite],
  );
...
interface DetailProps {
  row: CheckoutRow;
  invitations: models.CheckoutInvitation[];
  visitRecordCount: number | null;
  onReturn: () => void;
  onForceReturn: () => void;
  onReassign: () => void;
  onInvite: () => void;
  onRevokeInvite: (inv: models.CheckoutInvitation) => void;
                ...
                >
                  ...
                  {!revoked && (
                    <span className="checkouts-invitation-meta">
                      {formatRemaining(String(inv.expiresAt))}
                    </span>
                  )}
                </div>
                {!revoked && isActive && (
