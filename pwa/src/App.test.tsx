...
import { DEV_SEED_USERS, DevIdentityService } from "./services/identity-service";
...
describe("App", () => {
  ...
  it("既定アクター（admin）は管理者向けメニューが見える", async () => {
    await renderApp();
    expect(await screen.findByRole("link", { name: /メンバー管理/ })).toBeInTheDocument();
  ...
  it("member アクターには editor 以上のメニューが見えない", async () => {
    ...
    expect(screen.queryByRole("link", { name: /チェックアウト管理/ })).toBeNull();
  });
  ...
  it("設定画面でアイデンティティを member に切替えるとメニューが絞られる", async () => {
    await renderApp();
    await userEvent.click(await screen.findByRole("link", { name: /設定/ }));
    await userEvent.click(await screen.findByRole("radio", { name: /Dev Member/ }));
    expect(screen.queryByRole("link", { name: /メンバー管理/ })).toBeNull();
