// desktop/frontend/src/services/region-service.ts からの移植。
...
export interface RegionBindingAPI {
  ...
  GetParentArea(id: string): Promise<ParentArea>;
...
export class RegionService {
  ...
  async addParentArea(regionId: string, name: string): Promise<void> {
    ...
    const pa: ParentArea = { id, regionId, number, name, geometry: null };
  ...
  async addArea(parentAreaId: string): Promise<void> {
    ...
    const area: Area = { id, parentAreaId, number, geometry: null };
  ...
  async renameParentArea(id: string, name: string): Promise<void> {
    ...
    await this.api.SaveParentArea({ ...pa, name });
  }
