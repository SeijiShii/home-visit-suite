// desktop/frontend/src/pages/CoveragePage.tsx からの移植。
// Wails Binding 依存を useServices() に置き換えた。
// - AvailablePeriodBinding → availablePeriodService
// - RegionBinding → regionRepo
// サービスの日付引数は ISO 8601 文字列、UpdatePeriod は PeriodUpdate オブジェクト。
...
function periodPhase(p: AvailablePeriod, now: Date): Phase {
...
export function CoveragePage() {
  ...
  const [activePeriod, setActivePeriod] = useState<AvailablePeriod | null>(
    null,
  );
  const [tags, setTags] = useState<AvailablePeriodTag[]>([]);
  const [parentAreas, setParentAreas] = useState<ParentAreaOption[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showCreate, setShowCreate] = useState(false);
  const [editTarget, setEditTarget] = useState<AvailablePeriod | null>(null);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const [list, active, tagList, regions] = await Promise.all([
        services.availablePeriodService.listPeriods(),
        services.availablePeriodService.getActivePeriod(new Date()),
        services.availablePeriodService.listTags(),
        services.regionRepo.listRegions(),
      ]);
      setPeriods(list);
      ...
      for (const r of regions) {
        const pas = await services.regionRepo.listParentAreas(r.id);
        for (const pa of pas) {
          paOptions.push({
            id: pa.id,
            number: pa.number,
  ...
  const handleDelete = async (period: AvailablePeriod) => {
    if (!window.confirm(c.confirmDelete({ name: period.name }))) return;
    try {
      await services.availablePeriodService.deletePeriod(
        currentActorID,
        period.id,
      );
      await reload();
  ...
  const handleDeleteTag = async (tag: AvailablePeriodTag) => {
    const count = periods.filter((p) => p.tagIds.includes(tag.id)).length;
    if (!window.confirm(c.confirmDeleteTag({ name: tag.name, count }))) return;
    try {
      await services.availablePeriodService.deleteTag(currentActorID, tag.id);
      await reload();
    } catch (e) {
      setError(String(e));
    }
    ...
    <div className="coverage-page">
      ...
      {activePeriod && (
        <section className="coverage-active-period">
          ...
          <div>
            ...
            <span>
              {activePeriod.startDate.slice(0, 10)} -{" "}
              {activePeriod.endDate.slice(0, 10)}
            </span>
          </div>
        </section>
      )}
              ...
              {periods.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>
                      <span
                        className={`coverage-phase coverage-phase-${phase}`}
                      >
                        {phaseLabel(phase)}
                      </span>
                    </td>
      ...
      {showCreate && (
        <PeriodDialog
          ...
          tags={tags}
          apSvc={services.availablePeriodService}
...
function PeriodDialog({
  ...
  const [startDate, setStartDate] = useState(
    period ? period.startDate.slice(0, 10) : today,
  );
  const [endDate, setEndDate] = useState(
    period ? period.endDate.slice(0, 10) : today,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        const update: PeriodUpdate = {};
        if (phase === "pending") {
          update.name = name;
          update.startDate = startIso;
          update.endDate = endIso;
        ...
        await apSvc.updatePeriod(actorId, period.id, update);
      }
      await onSaved();
    } catch (e) {
      onError(String(e));
...
function AddTagDialog({
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    try {
      await apSvc.createTag(actorId, name, color);
      await onSaved();
    } catch (e) {
      onError(String(e));
      setSubmitting(false);
