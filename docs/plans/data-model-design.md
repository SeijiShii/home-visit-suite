...
| レイヤー | 用途 | 同期範囲 | API |
|---|---|---|---|
| **DeviceDB** | 同一メンバーのデバイス間同期 | 個人のみ | `Put/Get/Delete/List` by table |
| **GroupShare** | 異なるメンバー間の共有 | グループメンバー | `Put/Get/Delete/List` by channel × topic |
...
| `places` | `{areaId}` | 場所（住宅情報） | editor+（活動メンバーは申請経由） | 永続 |
| `users` | `roster` | メンバー名簿・ロール | admin | 永続 |
| `org_groups` | `roster` | 組織グループ・所属 | admin | 永続 |
| `member_tags` | `roster` | メンバータグ定義 | editor+ | 永続 |
| `teams` | `{regionId}` | チーム | editor+, 活動メンバー | 永続 |
| `activities` | `{areaId}` | 訪問活動（チェックアウト） | editor+（持ち出しは活動メンバー） | 管理者設定 |
| `activity_assignments` | `{activityId}` | チーム割り当て | editor+ | 管理者設定 |
| `visit_records` | `{areaId}` | 訪問記録（共有部分） | 活動メンバー（自分の記録） | 管理者設定 |
| `visit_record_edits` | `{visitRecordId}` | 訪問記録編集履歴 | editor+, 活動メンバー | 管理者設定 |
...
| `requests` | `{areaId}` | 活動メンバーからの申請 | 活動メンバー(作成), editor+(処理) | 管理者設定 |
...
活動メンバーアプリはチェックアウト中の区域に関連するtopicのみSubscribeすることで、必要最小限のデータだけを同期できる。例えば、区域 `NRT-001-05` をチェックアウトした活動メンバーは以下をSubscribeする：
...
- 仕様(04)「編集メンバーは他のメンバーにタグを付与」— `TagIDs`が必要
...
type Activity struct {
    ...
    LentByID       string         `json:"lentById"`       // ★追加: 貸し出した編集メンバー（持ち出し時は空）
...
- 仕様(05)「他のチームに貸し出す場合は編集メンバーが担当者」— `LentByID`
...
**変更理由**:
- 仕様(08)「活動メンバーは訪問時の家の人の反応などの情報を記述できる。これらは個人情報に当たるので他のメンバーに共有されない」
...
type Place struct {
    ...
    SortOrder      int        `json:"sortOrder"`      // ★追加: 並び順（編集メンバー変更可）
...
type AreaAvailability struct {
    ...
    StartDate      time.Time        `json:"startDate"`
    EndDate        time.Time        `json:"endDate"`
    SetByID        string           `json:"setById"`      // 設定した編集メンバー
    CreatedAt      time.Time        `json:"createdAt"`
}
```
...
領域間のアクセス制御は不要。topicベースのSubscriptionによるトラフィック最適化で十分。活動メンバーは自分が関与する領域のtopicのみSubscribeする。
...
- デフォルトは辞書順ソートだが、編集メンバーが並び順を変更可能（`SortOrder int`）
...
アプリ起動時にNTP同期チェックを行い、5分以上のずれを検出した場合はメンバーに警告する。
...
| **Phase 4** | メンバー管理・通知 | Invitation, Notification, Request, AuditLog | Phase 1 |
