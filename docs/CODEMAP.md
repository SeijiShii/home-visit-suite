...
- `data/linkself/sql-shared-storage.ts` — groupshare 共有レコード（catch-up 高水位・LWW 判定材料・catch-up 応答の再送元）のグループ DB(SQL) 永続ストア（shared_records テーブル・旧 localStorage キーの一度きり移行＝quota 解放。docs/wants/01「共有レコードストアの SQL 化」）(→01,11)
...
- `lib/linkself/shared-store.ts` — membership epoch（LocalStorageEpochStore、スナップショット巻き戻り防止）の localStorage 永続。共有レコード側（LocalStorageSharedStorage）は quota 黙殺欠損のため data/linkself/sql-shared-storage.ts に置換済み（旧キーの移行読み取り形式の定義元として残置）(→01)
