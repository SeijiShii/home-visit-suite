...
- `lib/linkself/client-factory.ts` — ブラウザ用 LinkSelf クライアント（@linkself/core）の組み立て。WebSocket + Circuit Relay v2（`circuitRelayTransport`）/Noise/yamux + identify サービスで libp2p を構成し `createLinkSelfClient()` が起動済み `LinkSelfSession`（client + libp2p + graceful stop）を返す。circuit-relay により着信不可のブラウザ同士がリレー経由（`/p2p-circuit`）で相互到達できる。`transportPrivateKey`（省略時 DID 鍵）で libp2p host 鍵を DID 鍵から分離＝同一 DID 多端末が別 peerId を持ち相互 devicesync 可能に。`sqlDatabase`（OPFS SqliteWasmDatabase）を渡すと `client.myDB` の SQL/KV が使える。M5 統合の実利用エントリ
- `lib/linkself/device-key.ts` — この端末固有の libp2p transport 鍵を生成・永続（32byte シードを localStorage `hvs.deviceKeySeed` に保管、決定的復元で peerId 固定）。`loadOrCreateDeviceTransportKey()`。DID 鍵（hvs.identity）とは独立。マルチデバイス devicesync の端末別 peerId を供給（link-self mutualAuth と対）
