...
- ブラウザピアの通信を成立させる 3 役を Go デーモンが担えるようにする:
  ...
  - **メールボックス（store-and-forward）**: オフラインピア宛メッセージの預かり
    - **ロスターメールボックス（2026-07-17 実装済み）**: メールボックスを「ユーザー DID→署名済み・封緘ロスター」のランデブーにも使う（どの兄弟端末の QR からでも全兄弟へ到達・失効の恒久配送。link-self spec `network-invitation.md` §7.6 が SoT、`01_共通基盤.md`「ロスターメールボックス」参照）
