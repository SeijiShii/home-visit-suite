...
type RegionService interface {
	...
	CreateRegion(actorID string, name, symbol string) (*domain.Region, error)

	// CreateParentArea は区域親番を作成する（編集スタッフ以上）。
	CreateParentArea(actorID string, regionID, number, name string) (*domain.ParentArea, error)

	// CreateArea は区域を作成する（編集スタッフ以上）。
	CreateArea(actorID string, parentAreaID, number string) (*domain.Area, error)
